...
class UNetSpatioTemporalConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        if self.controlnet_enabled and down_block_additional_residuals is not None:
            ...
            # Check if dwon block res samples and down block additional residuals are the same length
            if len(down_block_res_samples) != len(down_block_additional_residuals):
                raise ValueError(
                    f"Down block res samples length {len(down_block_res_samples)} and down block additional residuals length {len(down_block_additional_residuals)} are not the same"
                )
