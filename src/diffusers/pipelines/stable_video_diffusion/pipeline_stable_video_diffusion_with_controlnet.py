...
class CustomConditioningNet(ModelMixin, ConfigMixin ):
    ...
    def forward(self, x, do_classifier_free=False):
        ...
        if do_classifier_free:
            negative_embedding = torch.zeros_like(x)
            x = torch.cat([negative_embedding, x])
        
        return x

class wrapperModel(nn.Module):
    def __init__(self,customConditioningNet : CustomConditioningNet, model : UNetSpatioTemporalConditionModel):
        super().__init__()
...
class SpatioTemporalControlNet(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if controlnet_condition is not None:
            ...
            do_classifier_free = True if sample.shape[0] == 28 else False
            # print("this is classifier free", do_classifier_free)
            controlnet_condition =  self.conditioning_embedding.forward(controlnet_condition, do_classifier_free=do_classifier_free)
...
class StableVideoDiffusionPipelineWithWrapper(DiffusionPipeline):
    r"""
    Pipeline to generate video from an input image using Stable Video Diffusion.
    ...
    def __init__(
        ...
        scheduler: EulerDiscreteScheduler,
        feature_extractor: CLIPImageProcessor,
        wrapper: wrapperModel # Het is niet echt van dit format 
    ):
        super().__init__()
    ...
    def __call__(
        ...
        num_frames = num_frames if num_frames is not None else 14 
        decode_chunk_size = decode_chunk_size if decode_chunk_size is not None else num_frames
        ...
        # It seems that image_embeddings determines the device and dtype and device
        self.wrapper.cast_model_to(device=device, dtype = image_embeddings.dtype)
